...
def synthesize(analytics: dict[str, Any], ml_runs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in analytics.get("insights", []):
        ...
        if item.get("type") == "DATA_QUALITY" and item.get("severity") == "WARNING":
            actions.append({"priority": "HIGH", "action": "Review the affected field and correct or document the underlying data-quality issue before using it for downstream decisions.", "basis": _text(item.get("title"), "Data-quality warning"), "constraint": "This is a data remediation action, not an employee-level decision."})
    ...
    for result in ml_runs:
        ...
        if objective == "anomaly_detection":
            ...
            if not 0 <= share <= 1:
                rejected_runs += 1
                continue
            evidence.append({"source": "anomaly_detection", "type": "ANOMALY", "severity": "INFO", "title": f"{share * 100:.1f}% of usable rows flagged for review", "evidence": f"{_text(result.get('method'), 'Anomaly detection')} identified multivariate outlier patterns across confirmed predictors; usable rows: {result.get('rows_used', 0):,}."})
    ...
    return {"agent": "bounded_evidence_synthesizer_v3", "plan": ["collect verified findings", "validate supported objectives and dataset provenance", "classify evidence by analytical source", "rank material signals", "draft reversible investigation actions", "attach limitations"], "evidence": evidence, "recommendations": actions, "limitations": limitations, "raw_hr_records_accessed": False}
