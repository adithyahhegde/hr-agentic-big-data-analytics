...
## External Spark operator handoff
The remaining empirical gate is tracked in GitHub issue #7. `docs/EXTERNAL_SPARK_VALIDATION_RUNBOOK.md` defines the target requirements, dispatch procedure, admissible evidence, and the explicit blocker. This documentation does not mark the gate complete: only a successful run against an independently operated non-local Spark target can do that.
