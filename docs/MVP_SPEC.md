...
6. Collision detection; collisions are blocking and require review.
7. A small browser interface for upload, status, profile, mapping proposal, and warning states.

## Explicitly deferred

Persistent datasets/runs, user mapping confirmation, local-LLM calls, Spark routing, objective feasibility, agent planning, ML, SHAP, reports, and recommendations. The contracts are documented now so these stages can be added without changing the foundation’s meaning.
