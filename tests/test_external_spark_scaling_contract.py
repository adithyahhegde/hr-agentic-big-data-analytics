...
def _artifact(tmp_path: Path, **overrides):
    runs = [
        {"rows": 100, "fixture_sha256": "a" * 64, "fixture_schema": ["employee_id"], "elapsed_seconds": 1.0, "rows_per_second": 100.0, "validation": {k: True for k in ("row_count_matches_fixture", "distributed", "raw_rows_returned", "aggregates_match_local_baseline", "spark_version_present", "parallelism_positive", "application_id_present")}, "result": {"execution": {"spark_version": "3.5.8", "default_parallelism": 2, "application_id": "app-1", "input_mode": "driver_parallelized_csv"}}},
        {"rows": 1000, "fixture_sha256": "b" * 64, "fixture_schema": ["employee_id"], "elapsed_seconds": 2.0, "rows_per_second": 500.0, "validation": {k: True for k in ("row_count_matches_fixture", "distributed", "raw_rows_returned", "aggregates_match_local_baseline", "spark_version_present", "parallelism_positive", "application_id_present")}, "result": {"execution": {"spark_version": "3.5.8", "default_parallelism": 2, "application_id": "app-2", "input_mode": "driver_parallelized_csv"}}},
    ]
